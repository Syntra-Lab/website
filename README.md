# Syntra Research Documentation

Research documents on mathematical frameworks, theological interpretation systems, and technical design patterns.

## Core Research

- [Kalima Mathematical Framework](Kalima%20Mathematical%20Framework.html)
- [Limits of the Mathematics](Limits%20of%20the%20Mathematics.html)
- [Methodology](METHODOLOGY.html)
- [Literature Review: Quranic Arabic NLP & ML](literature_review_quran_arabic_nlp_ml.html)

## Technical Design

- [Technical Design Document](Technical%20Design%20Document/index.html)

## Export

Print any HTML document to PDF from your browser for best Arabic/math rendering (`Ctrl+P` / `Cmd+P`).

## Zenodo

Archived versions are automatically uploaded to [Zenodo](https://zenodo.org) as drafts on changes. Publish manually when ready.
